#!/usr/bin/env node
/**
 * GitHub Actions "Android App Release" tetikler, bitene kadar bekler.
 *
...
async function waitForRun(repo, beforeRunId) {
  ...
  process.stdout.write('[android:ci] Derleme bekleniyor');
...
async function main() {
  ...
  console.log(`[android:ci] Workflow tetikleniyor (${workflowFile})…`);
  ...
  try {
    const rel = sh(`gh release view ${tag} --repo ${repo} --json url,assets -q '.url' 2>/dev/null || true`);
    if (rel) {
      console.log(`[android:ci] Release: ${rel}`);
      const assetUrl = sh(
        `gh release view ${tag} --repo ${repo} --json assets -q '.assets[0].url' 2>/dev/null || true`,
      );
      if (assetUrl) console.log(`[android:ci] APK asset: ${assetUrl}`);
    }
  } catch {
    console.log('[android:ci] Tag release yok; workflow artifact kullanın.');
  }
}
