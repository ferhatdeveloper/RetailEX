...
INSERT INTO rex_001_products (code, name, name2, category_id, vat_rate, price, stock, is_active) VALUES
...
SELECT s.id, p.ref_id, 1, 45000.00, 20, 45000.00, 9000.00, 54000.00
FROM rex_001_01_sales s, rex_001_products p
WHERE s.fiche_no = 'SAT-2026-0001' AND p.code = 'PHONE-001'
  AND NOT EXISTS (SELECT 1 FROM rex_001_01_sale_items WHERE sale_ref = s.id AND product_ref = p.ref_id);

INSERT INTO rex_001_01_sale_items (sale_ref, product_ref, amount, price, vat_rate, total_net, total_vat, total_gross)
SELECT s.id, p.ref_id, 1, 95000.00, 20, 95000.00, 19000.00, 114000.00
FROM rex_001_01_sales s, rex_001_products p
WHERE s.fiche_no = 'SAT-2026-0002' AND p.code = 'LAPTOP-001'
  AND NOT EXISTS (SELECT 1 FROM rex_001_01_sale_items WHERE sale_ref = s.id AND product_ref = p.ref_id);

INSERT INTO rex_001_01_sale_items (sale_ref, product_ref, amount, price, vat_rate, total_net, total_vat, total_gross)
SELECT s.id, p.ref_id, 2, 450.00, 10, 900.00, 90.00, 990.00
FROM rex_001_01_sales s, rex_001_products p
WHERE s.fiche_no = 'SAT-2026-0003' AND p.code = 'CLOTH-M-001'
  AND NOT EXISTS (SELECT 1 FROM rex_001_01_sale_items WHERE sale_ref = s.id AND product_ref = p.ref_id);

INSERT INTO rex_001_01_sale_items (sale_ref, product_ref, amount, price, vat_rate, total_net, total_vat, total_gross)
SELECT s.id, p.ref_id, 1, 380.00, 10, 380.00, 38.00, 418.00
FROM rex_001_01_sales s, rex_001_products p
WHERE s.fiche_no = 'SAT-2026-0003' AND p.code = 'CLOTH-W-001'
  AND NOT EXISTS (SELECT 1 FROM rex_001_01_sale_items WHERE sale_ref = s.id AND product_ref = p.ref_id);

INSERT INTO rex_001_01_sale_items (sale_ref, product_ref, amount, price, vat_rate, total_net, total_vat, total_gross)
SELECT s.id, p.ref_id, 16, 15.00, 10, 240.00, 24.00, 264.00
FROM rex_001_01_sales s, rex_001_products p
...
INSERT INTO rex_001_01_cash_lines (fiche_no, date, amount, trcode, definition)
SELECT v.fiche_no, v.date::TIMESTAMPTZ, v.amount, v.trcode, v.definition
FROM (VALUES
