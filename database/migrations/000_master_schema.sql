...
-- Rapor Oluşturucu — kayıtlı SELECT raporları
...
CREATE TABLE IF NOT EXISTS public.customer_call_plan_weekly (
  ...
  call_plan_note TEXT,
  call_plan_caller_user_id UUID,
  call_plan_caller_name TEXT,
...
BEGIN
  ...
  EXECUTE format('CREATE TABLE IF NOT EXISTS public.%I (id UUID PRIMARY KEY DEFAULT gen_random_uuid(), firm_nr VARCHAR(10) NOT NULL, order_no VARCHAR(50) UNIQUE, recipe_id UUID NOT NULL, product_id UUID NOT NULL, planned_qty DECIMAL(15,3) NOT NULL, produced_qty DECIMAL(15,3) DEFAULT 0, status VARCHAR(20) DEFAULT ''draft'', start_date DATE, end_date DATE, completed_at TIMESTAMPTZ, note TEXT, purchase_invoice_id UUID, purchase_invoice_no VARCHAR(80), supplier_id UUID, supplier_name VARCHAR(255), created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP);', v_prefix || '_production_orders');
  -- 104: Genel üretim emirlerine alış faturası bağlantısı (çift belge engeli)
  EXECUTE format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS purchase_invoice_id UUID', v_prefix || '_production_orders');
  EXECUTE format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS purchase_invoice_no VARCHAR(80)', v_prefix || '_production_orders');
  EXECUTE format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS supplier_id UUID', v_prefix || '_production_orders');
  EXECUTE format('ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS supplier_name VARCHAR(255)', v_prefix || '_production_orders');
...
-- ============================================================================
-- 13) Müşteri satın alma geçmişi yardımcı fonksiyonları (126)
-- ============================================================================
-- 126_customer_purchase_history_view.sql ile aynı — sıfır kurulumda da hazır.
-- İdempotent: CREATE OR REPLACE FUNCTION. Sıralı yeniden yükleme güvenli.
-- ============================================================================
