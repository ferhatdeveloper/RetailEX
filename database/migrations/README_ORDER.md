...
115. **141_backfill_missing_cash_lines_for_sales.sql** – 31 Ağustos 2026'dan beri cash satış faturalarında (trcode 7/8/4/9, payment_method 'cash'/'nakit', net_amount > 0, is_cancelled=false) cash_lines tablosunda karşılığı olmayan 58 satırı MERKEZ KASA'ya geriye dönük ekler (currency_code='YEREL', sign=+1, transaction_type='KASA_GIRIS'). İdempotent: NOT EXISTS korur. Kasa bakiyesi delta'sı tek UPDATE ile uygulanır. Kök neden: `writeCashRegisterLineForInvoice` create sırasında sessizce `console.warn` ile yutulan bir hata alıyordu (olası UNIQUE(fiche_no) yarış durumu veya geçici DB timeout). Kod tarafında 3 denemeli retry + UNIQUE ihlalinde UPDATE fallback eklendi (`src/services/api/invoices.ts`).
116. **142_cash_lines_trigger_for_sales.sql** – `sales` tablosuna AFTER INSERT/UPDATE OF trigger; `payment_method IN ('cash','nakit','kasa',...)` VE `fiche_type IN ('sales_invoice','sales','satis','service',...)` VE `status <> 'cancelled'` VE `is_cancelled = false` VE `net_amount > 0` koşullarında otomatik `cash_lines` INSERT + `cash_registers.balance += amount`. UNIQUE(fiche_no) çakışmasında mevcut satır UPDATE edilir (idempotent, balance değişmez). Hedef kasa: header_fields.cash_register_id → MERKEZ KASA / PATRON KASA tercihli aktif kasa. Frontend `writeCashRegisterLineForInvoice` hâlâ yazıyor; bu trigger **ek savunma katmanı** — JS hangi build olursa olsun, Vercel/Dockge deploy olmasa bile yeni nakit satışlar kasaya yansır (2026-09-01 skandalı kalıcı çözümü).

117. **143_drop_auto_cash_line_trigger.sql** – 142 ile kurulan `trg_auto_cash_line_sales` trigger'ı ve `public.fn_auto_cash_line_on_sale()` fonksiyonu kaldırılır. Önceden backfill edilmiş `cash_lines` kayıtları (141 ve 142 ile eklenen) KORUNUR. Kullanıcı geri bildirimi: trigger yerine kod tarafında (Market POS ödeme modalı pattern'i) çözüm istedi. Kod güvencesi: `writeCashRegisterLineForInvoice` artık `db` (postgres.query) + `rest_api` (postgrest.post/patch) çift yolunu destekler, 3 denemeli retry + UNIQUE(fiche_no) → UPDATE fallback içerir (`src/services/api/invoices.ts`).
...
- `153_system_settings_report_menu_params.sql` — `system_settings.report_menu_params` (JSONB); güzellik anket/aranmayanlar + sanal santral (`virtual-pbx-caller-id`) + fiyat değişimi (`stock-price-change-slips`) menü görünürlüğü; varsayılan kapalı.
