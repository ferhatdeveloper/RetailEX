      ...
      >
        ...
        <View style={styles.grid}>
          {QUICK_ACCESS.map((action) => (
            ...
            >
              <LinearGradientFallback colors={action.gradient} style={styles.actionCard}>
        ...
        <Text style={[styles.menuMeta, { color: colors.textMuted }]}>
          Menü: {counts.sections} grup · {counts.items} öğe · {counts.leaves} yaprak
        </Text>

        {/* Full menu like web sidebar */}
              ...
              <View key={item.id}>
                ...
                {expanded[item.id] && item.children
                  ? item.children.map((child) => (
                      ...
                      >
                        <Text style={{ color: colors.text, fontSize: 13, flex: 1 }}>{child.label}</Text>
...
const styles = StyleSheet.create({
  ...
  menuMeta: { fontSize: 11, marginTop: 4 },
  catTitle: {
    fontSize: 12,
    fontWeight: '800',
