...
import { campaignsTable, firmNr } from './erpTables';
...
function firmFilterSql(alias = ''): string {
  const p = alias ? `${alias}.` : '';
  return `(
    LPAD(TRIM(COALESCE(${p}firm_nr, '')), 3, '0') = $1
    OR TRIM(COALESCE(${p}firm_nr, '')) = $2
