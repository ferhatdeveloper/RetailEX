...
import { postgrestGet, postgrestPatch } from './postgrestClient';
...
function mapGibQueueRow(r: Record<string, unknown>): GibQueueRow {
  return {
    id: String(r.id ?? ''),
    ...
    doc_date: r.doc_date != null ? String(r.doc_date).slice(0, 10) : null,
    amount: Number(r.amount ?? 0) || 0,
    tax_amount: Number(r.tax_amount ?? 0) || 0,
    status: String(r.status ?? ''),
    error_message: r.error_message != null ? String(r.error_message) : null,
    gib_uuid: r.gib_uuid != null ? String(r.gib_uuid) : null,
  ...
  return runDataTransport({
    ...
    viaRest: async () => {
      const rows = await postgrestGet<Record<string, unknown>[]>(
        ...
        {
          select:
            'id,document_no,doc_type,customer_name,doc_date,amount,tax_amount,status,error_message,gib_uuid,created_at,sent_at,firm_nr,period_nr',
          period_nr: `eq.${pn}`,
      ...
      return (Array.isArray(rows) ? rows : [])
        .filter((r) => {
          ...
          return fr === rawFn || fr === paddedFn || fr === fn;
        })
        .map(mapGibQueueRow);
    },
    viaBridge: async () => {
      try {
        ...
        return res.rows.map((r) => mapGibQueueRow(r as unknown as Record<string, unknown>));
      } catch {
        return [];
      }
    },
...
export async function fetchGibQueueStats(): Promise<GibQueueStats> {
  ...
  const [rawFn, paddedFn] = firmMatchParams(fn);
  return runDataTransport({
    label: 'fetchGibQueueStats',
    viaRest: async () => {
      const rows = await fetchGibQueue(500);
      return {
        pending: rows.filter((r) => ['Beklemede', 'pending'].includes(r.status.trim())).length,
        sent: rows.filter((r) => ['Gönderildi', 'sent', 'Onaylandı'].includes(r.status.trim())).length,
        failed: rows.filter((r) => ['Reddedildi', 'rejected', 'failed'].includes(r.status.trim())).length,
        drafts: rows.filter((r) => ['Taslak', 'draft'].includes(r.status.trim())).length,
        total: rows.length,
      };
    },
    viaBridge: async () => {
...
export async function retryGibQueueItem(queueRowId: string): Promise<GibActionResult> {
  ...
  return runDataTransport({
    label: 'retryGibQueueItem',
    viaRest: () => retryGibQueueItemViaRest(id),
    viaBridge: () => retryGibQueueItemViaBridge(id),
  });
}

async function retryGibQueueItemViaRest(id: string): Promise<GibActionResult> {
  await postgrestPatch(
    `/gib_edocument_queue?id=eq.${encodeURIComponent(id)}`,
    { status: 'Beklemede', error_message: null, updated_at: new Date().toISOString() },
    { schema: 'public', prefer: 'return=minimal' },
  );
  return mockGibSendResult(id);
}

async function retryGibQueueItemViaBridge(id: string): Promise<GibActionResult> {
  await pgQuery(
...
async function mockGibSendResult(id: string): Promise<GibActionResult> {
  await delay(400);
  ...
  const response = {
    success: mockOk,
    message: mockOk ? 'E-Fatura mock gönderim başarılı (mobile)' : errorMessage,
    documentId: gibUuid,
  ...
  const patch = {
    status,
    gib_uuid: gibUuid,
    gib_response_json: response,
    error_message: errorMessage,
    sent_at: mockOk ? new Date().toISOString() : null,
    updated_at: new Date().toISOString(),
  };

  await runDataTransport({
    label: 'retryGibQueueItem.mockUpdate',
    viaRest: async () => {
      await postgrestPatch(
        `/gib_edocument_queue?id=eq.${encodeURIComponent(id)}`,
        patch,
  ...
  return {
    ok: mockOk,
    status,
    message: mockOk ? `Yeniden gönderildi (mock): ${status}` : errorMessage || 'Gönderim başarısız',
  };
}
...
export async function checkGibQueueStatus(queueRowId: string): Promise<GibActionResult> {
  ...
  return runDataTransport({
    label: 'checkGibQueueStatus',
    viaRest: () => checkGibQueueStatusViaRest(id),
    viaBridge: () => checkGibQueueStatusViaBridge(id),
  });
}

async function checkGibQueueStatusViaRest(id: string): Promise<GibActionResult> {
  const rows = await postgrestGet<Record<string, unknown>[]>(
    '/gib_edocument_queue',
    { id: `eq.${id}`, select: 'gib_uuid,status', limit: 1 },
    { schema: 'public' },
  );
  const row = Array.isArray(rows) ? rows[0] : null;
  if (!row) return { ok: false, status: '', message: 'Kayıt bulunamadı' };
  return mockGibStatusResult(id, row.gib_uuid != null ? String(row.gib_uuid) : null, String(row.status ?? ''));
}

async function checkGibQueueStatusViaBridge(id: string): Promise<GibActionResult> {
  ...
  const row = res.rows[0];
  if (!row) return { ok: false, status: '', message: 'Kayıt bulunamadı' };
  return mockGibStatusResult(id, row.gib_uuid, row.status);
}

async function mockGibStatusResult(
  id: string,
  existingUuid: string | null,
  _currentStatus: string,
): Promise<GibActionResult> {
  await delay(350);
  const statuses = ['Onaylandı', 'Beklemede', 'Reddedildi'] as const;
  const next = statuses[Math.floor(Math.random() * statuses.length)];
  const ok = next !== 'Reddedildi';
  const gibUuid = existingUuid || newUuid();
  const response = {
    success: ok,
    message: next,
    documentId: gibUuid,
  ...
  const patch = {
    ...
    gib_uuid: gibUuid,
    gib_response_json: response,
    error_message: ok ? null : 'Mock GİB: durum Reddedildi',
    updated_at: new Date().toISOString(),
  };

  await runDataTransport({
    label: 'checkGibQueueStatus.mockUpdate',
    viaRest: async () => {
      await postgrestPatch(
        `/gib_edocument_queue?id=eq.${encodeURIComponent(id)}`,
        patch,
  ...
  return { ok, status: next, message: `Durum: ${next}` };
}
