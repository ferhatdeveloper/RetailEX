...
export const rbacUiTranslations: Record<string, Entry> = {
  ...
  roleMgmtColUsers: { tr: 'Kullanıcı', en: 'Users', ar: 'المستخدمون', ku: 'بەکارهێنەر' },
  roleMgmtCopyTitle: { tr: 'Rolü kopyala', en: 'Copy role', ar: 'نسخ الدور', ku: 'ڕۆڵ کۆپی بکە' },
  roleMgmtCopySuffix: { tr: '(kopya)', en: '(copy)', ar: '(نسخة)', ku: '(کۆپی)' },
  roleMgmtCopySuccess: {
    tr: 'Rol kopyalandı',
    en: 'Role copied',
    ar: 'تم نسخ الدور',
  ...
  roleMgmtCopyError: {
    tr: 'Rol kopyalanamadı',
    en: 'Could not copy role',
    ar: 'تعذر نسخ الدور',
    ku: 'کۆپی سەرنەکەوت',
  },
  roleMgmtLandingLabel: { tr: 'Giriş ekranı', en: 'Landing', ar: 'شاشة الدخول', ku: 'چوونەژوورەوە' },
  roleMgmtEmpty: {
...
const mods: Record<string, { n: Entry }> = {
  ...
  'pos.discount': { n: M('POS indirim uygulama', 'POS apply discount', 'تطبيق خصم نقطة البيع') },
  'pos.refund': { n: M('POS iade / iade fişi', 'POS refund', 'مرتجع نقطة البيع') },
  'pos.cancel_sale': { n: M('POS satış iptali', 'POS cancel sale', 'إلغاء بيع نقطة البيع') },
  'pos.change_price': { n: M('POS fiyat değiştirme', 'POS change price', 'تغيير سعر نقطة البيع') },
  'pos.open_drawer': { n: M('POS kasa çekmecesi', 'POS open drawer', 'فتح درج النقد') },
  ...
  stockmovements: { n: M('Stok hareket fişleri', 'Stock movement slips', 'قيود حركة المخزون') },
  'store-transfer': { n: M('Depolar Arası Sevk', 'Inter-store transfer', 'النقل بين المستودعات') },
  warehousetransfer: { n: M('Depo transfer (menü)', 'Warehouse transfer (menu)', 'نقل المستودع') },
  'warehouse-definitions': { n: M('Ambar tanımları', 'Warehouse definitions', 'تعريفات المستودع') },
  purchase: { n: M('Satınalma Yönetimi', 'Purchasing', 'المشتريات') },
  'inventory-check': { n: M('Sayım ve Kontrol', 'Stock counts', 'الجرد') },
  stockcount: { n: M('Stok sayımı (menü)', 'Stock count (menu)', 'جرد المخزون') },
  'finance.cash': { n: M('Kasa Yönetimi', 'Cash management', 'إدارة الصندوق') },
  kasalar: { n: M('Kasalar (menü)', 'Cash registers (menu)', 'الصناديق') },
  'finance.bank': { n: M('Banka İşlemleri', 'Banking', 'العمليات البنكية') },
  banks: { n: M('Bankalar (menü)', 'Banks (menu)', 'البنوك') },
  ...
  invoices: { n: M('Fatura Yönetimi (genel)', 'Invoicing (general)', 'الفواتير') },
  salesinvoice: { n: M('Satış faturaları', 'Sales invoices', 'فواتير المبيعات') },
  purchaseinvoice: { n: M('Alış faturaları', 'Purchase invoices', 'فواتير الشراء') },
  salesreturn: { n: M('Satış iade faturaları', 'Sales return invoices', 'مرتجع المبيعات') },
  purchasereturn: { n: M('Alış iade faturaları', 'Purchase return invoices', 'مرتجع المشتريات') },
  'purchase-pricing': {
  ...
  'restaurant.smart_table': {
    n: M('Akıllı masa planı', 'Smart floor plan', 'مخطط الطاولات الذكي'),
  },
};
...
const descs: Record<string, Entry> = {
  ...
  'pos.discount': M('Satışta indirim uygulama yetkisi (üst POS EXECUTE da kapsar)', 'Apply discount at POS', 'صلاحية الخصم'),
  'pos.refund': M('POS üzerinden iade işlemi', 'Refund from POS', 'مرتجع من نقطة البيع'),
  'pos.cancel_sale': M('Açık veya tamamlanan satışı iptal', 'Cancel open/completed sale', 'إلغاء البيع'),
  'pos.change_price': M('Satır birim fiyatını değiştirme', 'Change line unit price', 'تغيير سعر السطر'),
  'pos.open_drawer': M('Nakit çekmecesini açma', 'Open cash drawer', 'فتح الدرج'),
  ...
  stockmovements: M('Menü: stok hareketleri ekranı', 'Menu: stock movements screen', 'شاشة حركات المخزون'),
  'store-transfer': M('Şubeler arası transfer ve onay süreci', 'Transfers between stores', 'النقل بين الفروع'),
  warehousetransfer: M('Menü screen_id: depo transfer', 'Menu screen: warehouse transfer', 'نقل المستودع'),
  'warehouse-definitions': M('Ambar / depo kartları tanımlama', 'Warehouse master data', 'تعريفات المستودع'),
  ...
  stockcount: M('Menü: stok sayımı', 'Menu: stock count', 'جرد المخزون'),
  'finance.cash': M('Nakit giriş/çıkış ve kasa bakiyeleri', 'Cash in/out & balances', 'حركة الصندوق'),
  kasalar: M('Menü: kasalar ekranı (finance.cash ile eşdeğer)', 'Menu: cash registers (= finance.cash)', 'شاشة الصناديق'),
  'finance.bank': M('Banka havale, eft ve pos işlemleri', 'Bank & POS movements', 'العمليات البنكية'),
  banks: M('Menü: bankalar ekranı (finance.bank ile eşdeğer)', 'Menu: banks (= finance.bank)', 'شاشة البنوك'),
  ...
  invoices: M('Tüm fatura türlerine genel erişim', 'General access to all invoice types', 'الوصول العام للفواتير'),
  salesinvoice: M('Satış faturası listesi ve oluşturma', 'Sales invoice list & create', 'فواتير المبيعات'),
  purchaseinvoice: M('Alış faturası listesi ve oluşturma', 'Purchase invoice list & create', 'فواتير الشراء'),
  salesreturn: M('Satış iade faturaları', 'Sales return invoices', 'مرتجع المبيعات'),
  purchasereturn: M('Alış iade faturaları', 'Purchase return invoices', 'مرتجع المشتريات'),
  'purchase-pricing': M(
  ...
  'restaurant.smart_table': M('Salon planı ve masa durumu', 'Floor plan & table status', 'مخطط الصالة'),
};
