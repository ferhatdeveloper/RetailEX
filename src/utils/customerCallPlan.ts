import { getLocalizedWeekdayLabels } from './dateLocale';
...
export function customerCallWeekdayLabel(
    value: unknown,
    locale: string = 'tr-TR',
    short: boolean = false,
): string {
    ...
    return getLocalizedWeekdayLabels(locale, short).find(d => d.value === normalized)?.label ?? '';
}

export function customerCallWeekdaysLabel(
    value: unknown,
    locale: string = 'tr-TR',
    short: boolean = false,
): string {
    return normalizeCustomerCallWeekdays(value)
        .map(day => customerCallWeekdayLabel(day, locale, short))
        .filter(Boolean)
        .join(', ');
}
