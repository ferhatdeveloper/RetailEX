...
const INVOICE_FIELD_META: Record<string, { category: TemplateFieldCategory; sample: string; description?: string }> = {
  ...
  '{{customerPhone2}}': { category: 'customer', sample: '+90 533 000 00 00' },
  '{{customerAddress}}': { category: 'customer', sample: 'Sanayi Mah. 5. Sok. No:3' },
  '{{customerTaxNo}}': { category: 'customer', sample: '9876543210' },
  '{{customerEmail}}': { category: 'customer', sample: 'info@ornek.com' },
  '{{customerCode}}': { category: 'customer', sample: 'C-001' },
  '{{customerCity}}': { category: 'customer', sample: 'İstanbul' },
  '{{customerFileNo}}': { category: 'customer', sample: '1248' },
  '{{fileId}}': { category: 'customer', sample: '1248' },
  '{{customerAge}}': { category: 'customer', sample: '34' },
  '{{customerBirthDate}}': { category: 'customer', sample: '12.03.1992' },
  '{{customerBirthDay}}': { category: 'customer', sample: '12' },
  '{{customerBirthMonth}}': { category: 'customer', sample: '03' },
  '{{customerBirthYear}}': { category: 'customer', sample: '1992' },
  '{{customerBirthYear2}}': { category: 'customer', sample: '92' },
  '{{customerOccupation}}': { category: 'customer', sample: 'Öğretmen' },
  '{{customerGender}}': { category: 'customer', sample: 'Female' },
  '{{customerTier}}': { category: 'customer', sample: 'VIP' },
  '{{customerNotes}}': { category: 'customer', sample: 'Alerji: lateks. Öncelikli randevu.' },
  '{{customerBalance}}': { category: 'customer', sample: formatNumber(0, 2, true) },
  '{{customerPoints}}': { category: 'customer', sample: '250' },
  '{{heardFrom}}': { category: 'customer', sample: 'Instagram' },
...
const INVOICE_EXTENDED: TemplateFieldDef[] = [
  ...
  field('{{customerFileNo}}', 'Dosya no', 'customer', '1248'),
  field('{{fileId}}', 'Dosya no (file_id)', 'customer', '1248'),
  field('{{customerPhone2}}', 'Telefon 2', 'customer', '+90 533 000 00 00'),
  field('{{customerAge}}', 'Yaş', 'customer', '34'),
  field('{{customerBirthDate}}', 'Doğum tarihi', 'customer', '12.03.1992'),
  field('{{customerBirthDay}}', 'Doğum günü (gg)', 'customer', '12'),
  field('{{customerBirthMonth}}', 'Doğum ayı (aa)', 'customer', '03'),
  field('{{customerBirthYear}}', 'Doğum yılı (yyyy)', 'customer', '1992'),
  field('{{customerBirthYear2}}', 'Doğum yılı (son 2 hane)', 'customer', '92'),
  field('{{customerOccupation}}', 'Meslek', 'customer', 'Öğretmen'),
  field('{{customerGender}}', 'Cinsiyet', 'customer', 'Female'),
  field('{{customerTier}}', 'Müşteri tipi', 'customer', 'VIP'),
  field('{{customerNotes}}', 'Müşteri notları', 'customer', 'Klinik not'),
  field('{{customerPoints}}', 'Puan', 'customer', '250'),
  field('{{heardFrom}}', 'Nereden duydu', 'customer', 'Instagram'),
...
export function buildDemoInvoicePreviewContext(): Record<string, unknown> {
  ...
  const customer = {
    ...
    customer_tier: 'vip',
    points: 250,
    notes: 'Alerji: lateks. Öncelikli randevu.',
  ...
  return {
    ...
    fileId: '1248',
    customerAge: '34',
    customerBirthDate: '12.03.1992',
    customerBirthDay: '12',
    customerBirthMonth: '03',
    customerBirthYear: '1992',
    customerBirthYear2: '92',
    customerOccupation: 'Öğretmen',
    customerGender: 'Female',
    customerTier: 'VIP',
    customerNotes: 'Alerji: lateks. Öncelikli randevu.',
    customerBalance: formatNumber(0, 2, true),
    customerPoints: '250',
    heardFrom: 'Instagram',
