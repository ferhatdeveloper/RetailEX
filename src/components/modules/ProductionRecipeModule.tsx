/**
 * Production Recipe Module — bağımsız klasik BOM + Üretim Emirleri ekranı
 *
 * Notlar:
 * - Kasap/karkas parçalama işlemleri bu modülde YOKtur; o iş `ButcherProductionModule` içinde durur.
 * - Burada sadece genel üretim reçetesi (mamul + hammaddeler + fire + maliyet) ve üretim emirleri yer alır.
 * - Eski `ProductionModule` içindeki 3-sekmeli yapıdan (kasap/eski parçalama/BOM) bu ekran
 *   "sadece BOM + Emirler" olarak ayrıştırıldı.
 */
...
import { productionAPI, ProductionOrder, ProductionRecipe } from '@/services/api/productionAPI';
import { ProductionService } from '@/services/productionService';
import { useProductStore } from '@/store/useProductStore';
import { cn } from '@/components/ui/utils';

export function ProductionRecipeModule() {
    ...
    <div className="h-full flex flex-col bg-slate-50">
      <div className="bg-slate-800 text-white px-6 py-4 flex justify-between items-center shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold tracking-tight">Üretim Reçeteleri</h2>
            <p className="text-xs text-slate-400">Mamul reçeteleri (BOM), fire oranı ve üretim emirleri</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-hidden p-6 flex flex-col gap-6">
        ...
        {loading && (
          <div className="text-[11px] text-slate-400 self-center italic">Yükleniyor…</div>
        )}
      </div>
    </div>
  );
...
function OrdersPanel({ orders, recipes, products, onRefresh, onComplete }: any) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6 h-full">
      <div className="space-y-4">
        <StatCard
          label="Bekleyen Emirler"
          value={orders.filter((o: ProductionOrder) => o.status === 'draft').length}
          color="bg-slate-100 text-slate-700"
          icon={<Clock className="w-4 h-4" />}
        />
        <StatCard
          label="Devam Edenler"
          value={orders.filter((o: ProductionOrder) => o.status === 'in_progress').length}
          color="bg-blue-100 text-blue-700"
          icon={<Play className="w-4 h-4" />}
        />
        <StatCard
          label="Tamamlananlar"
          value={orders.filter((o: ProductionOrder) => o.status === 'completed').length}
          color="bg-green-100 text-green-700"
          icon={<CheckCircle className="w-4 h-4" />}
        />

        <NewOrderDialog recipes={recipes} products={products} onCreated={onRefresh} />
      </div>
                  ...
                  <td className="px-4 py-3 text-right">
                    {order.status === 'draft' && (
                      <Button
                        ...
                        onClick={() => {
                          productionAPI.saveOrder({ id: order.id, status: 'in_progress' }).then(() => onRefresh());
                        }}
                      >
                        Başlat
                      </Button>
                    ...
                    {order.status === 'in_progress' && (
                      <CompleteOrderDialog order={order} onComplete={onComplete} />
                    )}
                  </td>
                </tr>
              ))}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {recipes.map((recipe: ProductionRecipe) => (
          <div
            key={recipe.id}
            className="bg-white border border-slate-200 rounded-xl shadow-sm hover:shadow-md transition-shadow group overflow-hidden"
          >
            <div className="p-4 border-b border-slate-100 flex justify-between items-start bg-slate-50/50">
              <div>
                <h4 className="font-semibold text-slate-900 group-hover:text-blue-600 transition-colors uppercase text-sm tracking-tight">
                  {recipe.name}
                </h4>
            ...
            <div className="p-4 space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-bold text-slate-900">
                  {recipe.totalCost.toLocaleString('tr-TR', { style: 'currency', currency: 'IQD' })}
                </span>
              </div>
              <div className="flex justify-between items-center">
...
function RecipeEditor({ recipe, products, onCancel, onSave }: any) {
  const [formData, setFormData] = useState<ProductionRecipe>(
    recipe || {
      name: '',
      productId: '',
      description: '',
  ...
  const updateIngQty = (id: string, qty: number) => {
    setFormData({
      ...
      ingredients: formData.ingredients.map(i =>
        i.materialId === id ? { ...i, quantity: qty } : i
      ),
    });
  };
  ...
  const removeIng = (id: string) => {
    setFormData({
      ...formData,
      ingredients: formData.ingredients.filter(i => i.materialId !== id),
    });
  };
        ...
        <div className="lg:w-1/3 space-y-6">
          ...
          <div className="p-4 bg-orange-50 border border-orange-100 rounded-xl">
            ...
            <p className="text-[11px] text-orange-700 leading-relaxed font-medium">
              Bu reçete ile yapılan tamamlama işlemleri otomatik olarak hammadde stoklarını düşürecek ve mamul stoğunu artıracaktır.
            </p>
          </div>
        </div>
          ...
          <div className="px-4 py-3 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 bg-slate-200 text-slate-600 rounded-full text-[10px]">
                {formData.ingredients.length} Kalem
              </span>
            </div>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <div className="max-h-60 overflow-auto border border-slate-100 rounded-lg divide-y divide-slate-50">
                    {filteredMaterials.map((p: any) => (
                      ...
                      >
                        <div>
                          <div className="text-xs font-bold text-slate-800 group-hover:text-blue-700 transition-colors uppercase">
                            {p.name}
                          </div>
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {formData.ingredients.length === 0 && (
                  <tr>
                    <td colSpan={6} className="py-20 text-center text-slate-400 italic text-xs">
                      Reçete içeriği henüz boş.
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-12">
            <div className="text-right">
              ...
              <div className="text-lg font-bold text-slate-900">
                {formData.totalCost.toLocaleString('tr-TR')} <span className="text-[10px] text-slate-500">IQD</span>
              </div>
            </div>
          </div>
        </div>
...
function NewOrderDialog({ recipes, products, onCreated }: any) {
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      const recipe = recipes.find((r: ProductionRecipe) => r.id === selectedRecipeId);
        ...
        <div className="space-y-5 pt-6">
          <div className="space-y-2">
            ...
            >
              ...
              {recipes.map((r: ProductionRecipe) => (
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          size="sm"
          className="h-7 text-[10px] font-bold bg-green-600 hover:bg-green-700 text-white rounded-md"
        >
          TAMAMLA
        </Button>
      </DialogTrigger>
...
export default ProductionRecipeModule;