import { X, Building, Search, Plus, Loader2, ExternalLink } from 'lucide-react';
...
export function InvoiceWorkplaceModal({ currentWorkplace, onSelect, onClose }: InvoiceWorkplaceModalProps) {
  ...
  const handleQuickCreate = async () => {
    ...
    try {
      ...
      await reload();
      handleSelect({ code: created.code, name: created.name });
  ...
  return (
    ...
    >
      ...
      <PercentBodyModalScrollBody className="p-4">
        {loaded && filteredWorkplaces.length === 0 && !showQuickAdd ? (
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className={`font-medium ${cardTitle}`}>{workplace.name}</p>
                      <p className={`text-sm ${cardSub}`}>
                        {tm('code')}: {workplace.code}
                      </p>
                    </div>
                    {isSelected && (
