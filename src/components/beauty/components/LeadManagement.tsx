...
const SOURCE_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string }> = {
    ...
    facebook:    { label: 'Facebook',   icon: Facebook,      color: 'bg-blue-100 text-blue-700' },
    instagram:   { label: 'Instagram',  icon: Instagram,     color: 'bg-pink-100 text-pink-700' },
    phone_call:  { label: 'Telefon',    icon: PhoneCall,     color: 'bg-purple-100 text-purple-700' },
    walk_in:     { label: 'Yüzyüze',   icon: MapPin,        color: 'bg-orange-100 text-orange-700' },
    referral:    { label: 'Tavsiye',    icon: Star,          color: 'bg-amber-100 text-amber-700' },
    other:       { label: 'Diğer',     icon: Users,         color: 'bg-slate-100 text-slate-600' },
};

const STATUS_CONFIG: Record<string, { label: string; color: string; next?: string }> = {
    new:                    { label: 'Yeni',             color: 'bg-blue-100 text-blue-700',   next: 'contacted' },
    contacted:              { label: 'İletişim Kuruldu', color: 'bg-indigo-100 text-indigo-700', next: 'qualified' },
    qualified:              { label: 'Nitelikli',        color: 'bg-purple-100 text-purple-700', next: 'appointment_scheduled' },
    appointment_scheduled:  { label: 'Randevu Alındı',  color: 'bg-amber-100 text-amber-700',  next: 'converted' },
    converted:              { label: 'Müşteriye Dönüştü', color: 'bg-green-100 text-green-700' },
        ...
        <div className="flex flex-col h-full bg-[#f8fafc] animate-in fade-in duration-500">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-900">Lead Yönetimi (CRM Pipeline)</h1>
                        <p className="text-xs text-slate-500 font-medium">
                            {isLoading ? 'Yükleniyor...' : `${leads.length} lead · ${leads.filter(l => l.status === 'converted').length} müşteriye dönüştü`}
                        </p>
                    </div>
                </div>
            ...
            <div className="flex-1 overflow-hidden flex p-4 gap-4">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-slate-50">
                        {isLoading ? (
                            ...
                            <div className="flex flex-col items-center justify-center h-40 gap-2 text-slate-400">
                                <Megaphone size={32} />
                                <p className="text-sm font-medium">{search ? 'Sonuç bulunamadı' : 'Henüz lead yok'}</p>
                            </div>
                        ) : filtered.map(lead => {
                                ...
                                <div key={lead.id} className="flex items-center gap-4 px-6 py-4 hover:bg-slate-50/80 transition-colors group cursor-pointer" onClick={() => openEdit(lead)}>
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        ...
                                        {lead.status === 'appointment_scheduled' && (
                                            ...
                                            >
                                                {converting === lead.id ? '...' : 'Müşteri Yap'}
                                            </button>
                                        )}
                                    </div>
                                </div>
                ...
                <div className="hidden lg:flex lg:w-2/5 flex-col gap-4 overflow-y-auto custom-scrollbar">
                    <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                        <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider mb-4">Pipeline Özeti</h3>
                    ...
                    <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                        <h3 className="text-sm font-black text-slate-800 uppercase tracking-wider mb-4">Kanal Dağılımı</h3>
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="bg-gradient-to-r from-pink-600 to-rose-500 p-6 text-white flex items-center justify-between">
                            <div>
                                <h2 className="text-lg font-black">{isEdit ? 'Lead Düzenle' : 'Yeni Lead'}</h2>
                        ...
                        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto custom-scrollbar">
                            <div>
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-1 block">Ad Soyad <span className="text-red-500">*</span></label>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-1 block">Kaynak</label>
                                ...
                                <div>
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-1 block">Aşama</label>
                            ...
                            {editing.status === 'lost' && (
                                <div>
                                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-wider mb-1 block">Kayıp Nedeni</label>
