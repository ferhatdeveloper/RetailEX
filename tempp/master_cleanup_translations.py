...
def extract_translations():
    ...
    for lang in langs:
        ...
        if match:
            ...
            if sidebar_match:
                sidebar_items = re.findall(r'([a-zA-Z0-9_]+)\s*:\s*(["\'].*?["\'])', sidebar_match.group(1))
            ...
            if menu_match:
                menu_items = re.findall(r'([a-zA-Z0-9_]+)\s*:\s*(["\'].*?["\'])', menu_match.group(1))
                for k, v in menu_items:
                    lang_data[lang]['menu'][k] = v
            
            # Root
            # Remove nested objects to simplify root extraction
            root_part = re.sub(r'sidebar: \{.*?\s+\},', '', block, flags=re.DOTALL)
            root_part = re.sub(r'menu: \{.*?\s+\},', '', root_part, flags=re.DOTALL)
            root_items = re.findall(r'^\s+([a-zA-Z0-9_]+)\s*:\s*(["\'].*?["\'])', root_part, re.MULTILINE)
            for k, v in root_items:
                lang_data[lang]['root'][k] = v

    # Standardize keys by taking union of all found keys and interface keys
    ...
    # Remove duplicates from all_root_keys that are also in menu/sidebar
