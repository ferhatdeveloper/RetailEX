...
# Set default encoding to utf-8 for print if needed
# but better write directly to file
...
def main():
    ...
    for k in en_keys_ordered:
        if k == 'menu':
            ...
            # Get all keys from both to be safe
            all_menu_keys = sorted(list(set(en_menu.keys()) | set(ku_menu.keys())))
            for mk in all_menu_keys:
                # Preference: ku_menu > en_menu
                val = ku_menu.get(mk, en_menu.get(mk, "'TODO'"))
    ...
    output = "\n".join(final_ku_lines)
    with open('d:/RetailEX/reconstructed_ku.txt', 'w', encoding='utf-8') as f:
        f.write(output)
