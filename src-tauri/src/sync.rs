...
pub async fn process_sync_queue_internal() -> Result<(), String> {
    ...
    let conn_str = format!(
        ...
        host_part, config.pg_local_user, config.pg_local_pass, db_name
    );
...
pub async fn enable_remote_support(app: tauri::AppHandle) -> Result<String, String> {
    ...
    if let Some(vpn_config) = config.vpn_config {
        let _ = crate::vpn::start_vpn_mesh(vpn_state, vpn_config).await;
    }
...
pub async fn get_last_sync_info(app: tauri::AppHandle) -> Result<SyncInfo, String> {
    ...
    // 3. Get pending count
    let rows = client.query("SELECT COUNT(*) FROM sync_queue WHERE status = 'pending'", &[]).await.map_err(|e| e.to_string())?;
    let pending_count: i64 = rows.first().map(|r| r.get(0)).unwrap_or(0);

    // 4. Get last success
    let rows_last = client.query("SELECT synced_at FROM sync_queue WHERE status = 'completed' ORDER BY synced_at DESC LIMIT 1", &[]).await.map_err(|e| e.to_string())?;
